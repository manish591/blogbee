...
import { addTagToPost, createPost, editPost, getAllPosts, getPostById } from "./posts.services";
...
describe("POSTS", () => {
  ...
  describe('POST /v1/posts', () => {
    ...
    it("should return 400 bad request if blogId is not provided in request body", async () => {
      const app = buildServer({ db });
      const data = {};
      const res = await request(app).post("/v1/posts").set("Accept", "application/json").set("Content-Type", "application/json").set("Cookie", [cookie]).send(data);
    ...
    it("should return 403 forbiddedn if user attempts to create post in the blog they do not own", async () => {
      ...
      const otherUserBlog = await createBlog(otherUserId, otherUserBlogData, db);
  ...
  describe("GET /v1/posts", () => {
    ...
    it("should return 400 bad request if invalid query params are passed", async () => {
      const app = buildServer({ db });
      const res = await request(app).get(`/v1/posts?blogId=${blogId}&invalidquery=data`).set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 400 bad request if blogId is not passed in query params", async () => {
      const app = buildServer({ db });
      const res = await request(app).get('/v1/posts').set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 403 forbidden if user attempts to read the posts of the blog they do not own", async () => {
      ...
      const otherUserBlogId = otherUserBlog.blogId.toString();
      const app = buildServer({ db });
      const res = await request(app).get(`/v1/posts?blogId=${otherUserBlogId}`).set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 200 ok along with the posts result for blog with blogId", async () => {
      ...
      await createPost(userId, blogId, db);
      const app = buildServer({ db });
      const res = await request(app).get(`/v1/posts?blogId=${blogId}`).set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 200 ok along with posts of blog with blogId, limit results to 2", async () => {
      ...
      await createPost(userId, blogId, db);
      const app = buildServer({ db });
      const res = await request(app).get(`/v1/posts?blogId=${blogId}&limit=2`).set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 200 ok along with posts of blog with blogId, return page 2 results with limit 3", async () => {
      ...
      await createPost(userId, blogId, db);
      const app = buildServer({ db });
      const res = await request(app).get(`/v1/posts?blogId=${blogId}&limit=3&page=2`).set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 200 ok along with posts of blog with blogId, return result for search query updated", async () => {
      ...
      const app = buildServer({ db });
      const res = await request(app).get(`/v1/posts?blogId=${blogId}&q=updated`).set("Accept", "application/json").set("Cookie", [cookie]);
  ...
  describe("GET /v1/posts/:postId", () => {
    ...
    it("should return 403 forbidden if user attempts to access the post they do not own", async () => {
      ...
      const app = buildServer({ db });
      const res = await request(app).get(`/v1/posts/${otherUserPostId}`).set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 200 ok along with post content", async () => {
      ...
      const app = buildServer({ db });
      const res = await request(app).get(`/v1/posts/${createdPostId}`).set("Accept", "application/json").set("Cookie", [cookie]);
  ...
  describe("PATCH /v1/posts/:postId", async () => {
    ...
    it("should return 400 bad request if invalid postId is passed in params", async () => {
      ...
      const app = buildServer({ db });
      const data = {};
      const res = await request(app).patch(`/v1/posts/${invalidPostId}`).set("Accept", "application/json").set("Cookie", [cookie]).send(data);
    ...
    it("should return 403 forbidden if user attempts to edit the post they do not own", async () => {
      ...
      const app = buildServer({ db });
      const res = await request(app).patch(`/v1/posts/${otherUserPostId}`).set("Accept", "application/json").set("Cookie", [cookie]).send(data);
    ...
    it("should return 200 ok for successfully editing the post", async () => {
      ...
      const res = await request(app).patch(`/v1/posts/${createdPostId}`).set("Accept", "application/json").set("Content-Type", "application/json").set("Cookie", [cookie]).send(data);
  ...
  describe("DELETE /v1/posts/:postId", async () => {
    ...
    it("should return 403 forbidden if user attempts to delete the posts that they do not own", async () => {
      ...
      const otherUserPostId = otherUserPost.postId.toString();
      const app = buildServer({ db });
      const res = await request(app).delete(`/v1/posts/${otherUserPostId}`).set("Accept", "application/json").set("Cookie", [cookie]);
      const otherUserEditedPost = await getPostById(otherUserPostId, db);
    ...
    it("should return 200 ok for successfuly deleting the post", async () => {
      ...
      const createdPostId = createdPost.postId.toString();
      const app = buildServer({ db });
      const res = await request(app).delete(`/v1/posts/${createdPostId}`).set("Accept", "application/json").set("Content-Type", "application/json").set("Cookie", [cookie]);
      const editedPost = await getPostById(createdPostId, db);
  ...
  describe("POST /v1/posts/:postId/tags/:tagId", () => {
    ...
    it("should return 403 forbidden if user attempts to add tag to the post they do not own", async () => {
      ...
      const otherUserPost = await createPost(otherUserId, otherUserBlogId, db);
      const otherUserPostId = otherUserPost.postId.toString();
      const otherUserTag = await createTag(otherUserId, otherUserBlogId, { name: "other user tag", description: null, blogId: otherUserBlogId }, db);
      const otherUserTagId = otherUserTag.tagId.toString();
      const app = buildServer({ db });
      const res = await request(app).post(`/v1/posts/${otherUserPostId}/tags/${otherUserTagId}`).set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 409 conflict if user attempts to add tag to the post where tag is already attached to post", async () => {
      ...
      const app = buildServer({ db });
      const res = await request(app).post(`/v1/posts/${createdPostId}/tags/${createdTagId}`).set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 403 forbidden is user attempts to add tag to the post where postId and tagId belongs to different blogs", async () => {
      ...
      const tagOnNewBlogData = { name: "new blog tag", description: null, blogId: newBlogId }
      const tagOnNewBlog = await createTag(userId, newBlogId, tagOnNewBlogData, db);
      const tagOnNewBlogId = tagOnNewBlog.tagId.toString();
      const app = buildServer({ db });
      const res = await request(app).post(`/v1/posts/${userPostOnOldBlogId}/tags/${tagOnNewBlogId}`).set("Accept", "application/json").set("Cookie", [cookie]);
    ...
    it("should return 200 ok for successfully adding tag to the post", async () => {
      const createdPost = await createPost(userId, blogId, db);
      const createdPostId = createdPost.postId.toString();
      const tagId = (await createTag(userId, blogId, { name: "new tag", description: null, blogId }, db)).tagId.toString();
      const app = buildServer({ db });
      const res = await request(app).post(`/v1/posts/${createdPostId}/tags/${tagId}`).set("Accept", "application/json").set("Cookie", [cookie]);
      const editedPost = await getPostById(createdPostId, db);
  ...
  describe("DELETE /v1/posts/:postId/tags/:tagId", () => {
    ...
    it("should return 403 forbidden if user attempts to delete the tag from the post they do not own", async () => {
      ...
      const otherUserPost = await createPost(otherUserId, otherUserBlogId, db);
      const otherUserPostId = otherUserPost.postId.toString();
      const otherUserTagId = await createTag(otherUserId, otherUserBlogId, { name: "other user tag", description: null, blogId: otherUserBlogId }, db).then(tag => tag.tagId.toString());
      addTagToPost(otherUserPostId, otherUserTagId, db);
      const app = buildServer({ db });
      const res = await request(app).delete(`/v1/posts/${otherUserPostId}/tags/${otherUserTagId}`).set("Accept", "application/json").set("Cookie", [cookie]);
      const updatedPostData = await getPostById(otherUserPostId, db);
    ...
    it("should return 404 not found if user attempts to delete the tag from the post where tag is not attached to the post", async () => {
      const createdPost = await createPost(userId, blogId, db);
      const createdPostId = createdPost.postId.toString();
      const createdTag = await createTag(userId, blogId, { name: "new tag", description: null, blogId }, db);
      const createdTagId = createdTag.tagId.toString();
      const app = buildServer({ db });
      const res = await request(app).delete(`/v1/posts/${createdPostId}/tags/${createdTagId}`).set("Accept", "application/json").set("Cookie", [cookie]);

...
describe('blogs', () => {
  ...
  describe('POST /v1/blogs', () => {
    ...
    it('should return 409 conflict if blog with slug already exists', async () => {
      ...
      const res = await request(app)
        ...
        .set('Cookie', [cookie])
        .send(data);
      const allBlogs = await getAllBlogsByUser(userId);
      const createdBlog = await getBlogBySlug(blogData.slug);
    ...
    it('should return 200 ok for successfully creating the new blog', async () => {
      ...
      const res = await request(app)
        ...
        .set('Cookie', [cookie])
        .send(data);
      const createdBlog = await getBlogBySlug(data.slug);
  ...
  describe('GET /blogs', () => {
    ...
    beforeEach(async () => {
      ...
      const newUserCreated = await createUser(userData);
      const newUserId = newUserCreated.userId.toString();
      await createBlog(newUserId, blogData5);
    });
  ...
  describe('GET /blogs/:blogId', () => {
    ...
    it('should return 200 ok along with blog data', async () => {
      ...
      const createdBlogResult = await createBlog(userId, blogData);
  ...
  describe('PATCH /blogs/:blogId', () => {
    ...
    it('should return 400 bad request if invalid request body is provided', async () => {
      ...
      const res = await request(app)
        ...
        .set('Cookie', [cookie])
        .send(data);
      const editedBlog = await getBlogById(blogId);
    ...
    it('should return 403 forbidden if user attempts to edit blog they do not own', async () => {
      ...
      const res = await request(app)
        ...
        .set('Cookie', [cookie])
        .send(blogDataToEdit);
      const otherBlogAfterEditing = await getBlogById(otherUserBlogId);
    ...
    it('should return 200 ok for successfully editing the blog content', async () => {
      ...
      const res = await request(app)
        ...
        .set('Cookie', [cookie])
        .send(data);
      const editedBlogData = await getBlogById(blogId);
